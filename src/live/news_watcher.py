...
class NewsWatcher:
    ...
    async def run(self):
        if self._news_col is not None:
            try:
                await self._news_col.create_index("url", unique=True, sparse=True)
        ...
        while True:
            try:
                ...
                events = []
            self.last_poll = utcnow().isoformat() + "Z"
            ...
            for news in events:
                ...
                if self._news_col is not None:
                    try:
                        await self._news_col.insert_one(news_to_doc(news, origin="live"))
                    except Exception:
                        pass
            if not events:
                logger.info("No new articles at %s", self.last_poll)
            else:
                logger.info("Fetched %d articles at %s", len(events), self.last_poll)
    ...
    async def _poll_concurrent(self):
        ...
        async def _safe_fetch(source):
            try:
                return await source.fetch_latest()
            except Exception as e:
                return e

        try:
            results = await asyncio.wait_for(
                asyncio.gather(*[_safe_fetch(s) for s in self.poller.sources]),
                timeout=30
            )
        ...
        for i, result in enumerate(results):
            ...
            for event in result:
                if self.poller._seen_col is not None:
                    if await self.poller._is_seen(event.url):
                        continue
                    await self.poller._mark_seen(event.url)
                events.append(event)
                count += 1
            if count:
